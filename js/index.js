...
function svgToPoints(svg) {
  ...
  for (let i = 0; i < svg.length; i++) {
    switch (svg[i]) {
      ...
      case "C":
        points.push({
          c: [{ x: svg[++i], y: svg[++i] }, { x: svg[++i], y: svg[++i] }],
          x: svg[++i],
          y: svg[++i]
        });
        break;
  ...
  console.log(points);
  return { points, pathClosed };
}

class Container extends Component {
  constructor(...args) {
    super(...args);
    let initData = document.getElementById("init-data").value;
    initData = svgToPoints(initData);
    console.log(initData);
    if (!initData) initData.points = [{ x: 100, y: 100 }];
    ...
    _defineProperty(this, "state", {
      ...
      activePoint: initData.points.length - 1,

      draggedPoint: false,
      draggedQuadratic: false,
      draggedCubic: false,
      closePath: initData.pathClosed
    });
    _defineProperty(
      this,
      "setWidth",
...
function Choices(props) {
  ...
  return React.createElement("div", { className: "ad-Choices" }, choices);
}
